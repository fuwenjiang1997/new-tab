    ...
    <div class="app-item-icon flex flex-col bg-neutral-700">
      ...
      <div class="flex flex-col bg-neutral-700" style="height: calc(100% - 40px)">
        <div class="flex-1 overflow-y-scroll no-scrollbar">
          <div class="flex gap-2 pt-2 px-4" v-for="item in alarmTasks">
    ...
    <div class="mt-5">
      <a-form :model="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
        ...
        <a-form-item label="通知日" name="weeks">
          <a-tag v-for="item in weekOptions" :key="item.value"
            @click="() => form.weeks[item.value] = !form.weeks[item.value]"
            :color="form.weeks[item.value] ? '#108ee9' : undefined" class="cursor-pointer">{{ item.label }}</a-tag>
        </a-form-item>
        ...
        <template v-if="form.notification">
          ...
          <a-form-item label="通知间隔" name="notificationRepeatTime">
            <div class="flex">
              <a-input-number class="!w-[90px] mr-10" v-model:value="form.notificationRepeatTime">
                <template #addonAfter>
          ...
          <a-form-item label="通知次数" name="notificationRepeatCount">
            <a-input-number class="!w-[200px]" v-model:value="form.notificationRepeatCount" placeholder="0或者空表示一直通知" />
          </a-form-item>
        </template>
      </a-form>
    </div>
...
function saveTask() {
  if (form.value.id) {
    const index = alarmTasks.value.findIndex(item => item.id === form.value.id)
